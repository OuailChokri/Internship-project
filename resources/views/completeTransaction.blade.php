...
<body>
    ...
    <div class="card" style="width: 100%;">
        <div class="card-header">
            <h5 class="mb-0 text-capitalize">Complete Transaction</h5>
        </div>
        <div class="card-body">
    <form action="{{route('transactions.update',$transaction->idTransaction)}}" method="post">
    @csrf
        <div class="mb-3">
            <label for="client" class="form-label text-secondary text-capitalize">Client</label>
            <select disabled class="form-select" id="client"name="client" required>
                <option>{{$transaction->client}}</option>
            </select>
        </div>
        <div class="mb-3">
            <label for="voiture" class="form-label text-secondary text-capitalize">Voiture</label>
            <select disabled class="form-select" id="voiture" name="voiture" required>
                <option>{{$transaction->voiture}}</option>
            </select>
        </div>
        <div class="mb-3">
            <label for="date_debut" class="form-label text-secondary text-capitalize">Date début</label>
            <input disabled value="{{$transaction->dateLocation}}" type="date" class="form-control" id="date_debut" name="dateDebut" required>
        </div>
        <div class="mb-3">
            <label for="date_retour" class="form-label text-secondary text-capitalize">Date de retour</label>
            <input disabled type="date" value="{{$transaction->dateRetour}}" class="form-control" id="date_retour" name="dateRetour" required>
        </div>
        <div class="mb-3">
            <label for="total_price" class="form-label text-secondary text-capitalize">Total Price</label>
            <input disabled type="text" value="{{$transaction->prix}}" class="form-control" id="amende_pd" name="prix" placeholder="" required></textarea>
        </div>
        <div class="mb-3">
            <label for="amende_pd" class="form-label text-secondary text-capitalize">Amende P.D</label>
            <input disabled type="text" value="{{$transaction->amende}}" class="form-control" id="amende_pd" name="amende" placeholder="" required></textarea>
        </div>
        <div class="mb-3">
            <label for="date_retour" class="form-label text-secondary text-capitalize">Retourner</label>
            <input  type="date" name="dateRetourEffective" class="form-control" id="date_retour" required>
        </div>
